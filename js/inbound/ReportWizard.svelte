<script lang="ts">
	...
	$effect(() => {
		if (wizard.step === 'system' && system === null)
			previewSystem()
				.then((value) => (system = value))
				.catch(Logger.error);
		if (wizard.step === 'build' && build === null)
			previewBuild()
				.then((value) => (build = value))
				.catch(Logger.error);
		if (wizard.step === 'log' && log === null)
			previewLogTail()
				.then((value) => (log = value))
				.catch(Logger.error);
		if (wizard.step === 'config' && config === null)
			previewConfig()
				.then((value) => (config = value))
				.catch(Logger.error);
		if (wizard.step === 'error' && error === null)
			previewError()
...
<div class="flex min-h-[420px] flex-col gap-4">
	...
	<div
		...
		aria-label={`Step ${progressPosition + 1} of ${progressSteps.length}`}
	>
		<div class="text-muted-foreground shrink-0 text-xs tabular-nums">
			Step {progressPosition + 1} of {progressSteps.length}
		</div>
	...
	{:else if wizard.step === 'message'}
		<section class="flex flex-1 flex-col gap-3">
			<label class="text-sm font-medium" for="inbound-message"
				>What happened? <span class="text-muted-foreground font-normal">(optional)</span></label
			>
			<textarea
				id="inbound-message"
				class="border-input bg-background min-h-32 resize-none rounded-md border p-3 text-sm"
				bind:value={wizard.message}
				placeholder="Add any context, expected behavior, or steps you tried."
			></textarea>
		</section>
	{:else if wizard.step === 'system'}
		<section class="flex flex-1 flex-col gap-3">
			<IncludeToggle
				bind:checked={wizard.include.system}
				title="System info"
				description="Operating system, CPU architecture, and locale."
			/>
			<pre class="bg-muted/50 max-h-48 overflow-auto rounded border p-3 text-xs">{JSON.stringify(
					system,
					null,
					2
				)}</pre>
	...
	{:else if wizard.step === 'build'}
		<section class="flex flex-1 flex-col gap-3">
			<IncludeToggle
				bind:checked={wizard.include.build}
				title="Build info"
				description="App version, git commit, and build timestamp."
			/>
			<pre class="bg-muted/50 max-h-48 overflow-auto rounded border p-3 text-xs">{JSON.stringify(
					build,
					null,
	...
	{:else if wizard.step === 'log'}
		<section class="flex flex-1 flex-col gap-3">
			<IncludeToggle
				bind:checked={wizard.include.log}
				title="Log tail"
				description="The most recent local app log lines, compressed before sending."
			/>
			{#if log?.exists}
				<div class="text-muted-foreground text-xs">
					{fmtBytes(log.byte_count)} raw, {fmtBytes(log.gzipped_size)} compressed
				</div>
				<pre
					class="bg-muted/50 max-h-48 overflow-auto rounded border p-3 text-xs whitespace-pre-wrap">{log.preview_text}</pre>
			{:else}
	...
	{:else if wizard.step === 'config'}
		<section class="flex flex-1 flex-col gap-3">
			<IncludeToggle
				bind:checked={wizard.include.config}
				title="Configuration snapshot"
				description="App settings with known secrets redacted."
			/>
			<pre class="bg-muted/50 max-h-48 overflow-auto rounded border p-3 text-xs">{config ??
					''}</pre>
		</section>
	{:else if wizard.step === 'error'}
		<section class="flex flex-1 flex-col gap-3">
			<IncludeToggle
				bind:checked={wizard.include.error}
				title="Detected error"
				description="The first captured error or panic from this session."
			/>
			<pre
				class="bg-muted/50 max-h-48 overflow-auto rounded border p-3 text-xs whitespace-pre-wrap">{JSON.stringify(
					error,
					null,
					2
				)}</pre>
		...
		<section class="flex flex-1 flex-col gap-3">
			...
			<div class="grid gap-2 text-sm">
				<div>
					...
					{wizard.discordUser || '(not provided)'}
				</div>
				<div>
					<span class="text-muted-foreground">Message:</span>
					{wizard.message || '(not provided)'}
				</div>
				<div>
	...
	{:else if wizard.step === 'sending'}
		<section class="text-muted-foreground grid flex-1 place-items-center text-sm">
			Sending report...
		</section>
		...
		<section class="grid flex-1 place-items-center gap-3 text-center">
			<div>
				...
				<p class="text-muted-foreground text-sm">
					Thanks. A private Discord thread was created for this report.
				</p>
			</div>
		</section>
	...
	<div class="flex items-center justify-between gap-2 border-t pt-3">
		<button
			type="button"
			class="border-input hover:bg-muted h-9 cursor-pointer rounded-md border px-3 text-sm disabled:cursor-not-allowed disabled:opacity-40"
			disabled={wizard.step === 'username' || wizard.step === 'sending' || wizard.step === 'done'}
			onclick={() => wizard.back()}
		>
			Back
		</button>
		{#if wizard.step === 'preview'}
			<button
				type="button"
				class="bg-primary text-primary-foreground h-9 cursor-pointer rounded-md px-3 text-sm disabled:cursor-not-allowed disabled:opacity-40"
				disabled={busy}
		...
		{:else if wizard.step === 'done'}
			<button
				type="button"
				class="bg-primary text-primary-foreground h-9 cursor-pointer rounded-md px-3 text-sm"
		...
		{:else if wizard.step === 'failed'}
			<button
				type="button"
				class="bg-primary text-primary-foreground h-9 cursor-pointer rounded-md px-3 text-sm"
				onclick={() => (wizard.step = 'preview')}>Try again</button
			>
		{:else}
			<button
				type="button"
				class="bg-primary text-primary-foreground h-9 cursor-pointer rounded-md px-3 text-sm disabled:cursor-not-allowed disabled:opacity-40"
				onclick={() => wizard.next()}>Next</button
			>
		{/if}
	</div>
</div>
